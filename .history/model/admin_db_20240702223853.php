...
function get_admin_by_username($username) {
    global $db;
    $query = 'SELECT * FROM adminstrators WHERE username = :username';
    $statement = $db->prepare($query);
    $statement->bindValue(':username', $username);
    $statement->execute();
...
function get_admin_email_by_username($username) {
    $admin = get_admin_by_username($username); // Using existing function to fetch account details
    if ($admin) {
        return $admin['email']; // Return email if account found
    } else {
