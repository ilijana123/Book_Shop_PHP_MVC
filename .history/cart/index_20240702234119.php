    ...
    case 'cart_view':
        ...
        case 'place_order':
            ...
            if (isset($_SESSION['cart'][$username]) && !empty($_SESSION['cart'][$username])) {
                ...
                $account = get_account($username); // Assuming $username is the username of the logged-in user
                $to = $account['email'];
                $subject = 'Order Confirmation';
                $message = 'Thank you for your order! Your books will be shipped soon.';
                
                // Check if $to (recipient email) is not empty
                if (!empty($to)) {
                    ...
                    // Handle case where recipient email ($to) is empty
                    echo "<script>
                            alert('Error: Recipient email address is empty. Please contact support.');
                            window.location.href = '../product_catalog';
                          </script>";
                }
                exit;
